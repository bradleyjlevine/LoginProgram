import java.math.BigInteger;
import java.util.Arrays;

public class AES
{
	private final short[] rconValues = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
	
	private final short[] sBoxValues = {};
	
	private final short[] inverseSbox = {};

	private short state[][] = new short[4][4];
	private short keys[][][] = new short[11][4][4];
	private short text[];
	private int start = 0;
	private int PAD_LEN = 0;
	private int NUM_BLOCKS = 1;
	private int currentBlock = 0;
	private boolean TEST = false;	//set to false if you want to get rid of test output and the use of the testKey
	
	private short[] testKey = {0x54, 0x68, 0x61, 0x74, 0x73, 0x20, 0x6D, 0x79, 0x20, 0x4B, 0x75, 0x6E, 0x67, 0x20, 0x46, 0x75};

	public AES()
	{
		super();
	
	}

	private void loadState(short[] array)
	{
		System.out.print("State loading from array at " + start);

		for(int i = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++, start++)
			{
				state[j][i] = array[start];
			}
		}
		
		System.out.println(" to " + start);
	}
	
	private void loadKey(short[] array)
	{

		for(int i = 0, k = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++, k++)
			{
				keys[0][j][i] = array[k];
			}
		}
	}

	private void generateKeys()
	{
		int round = 1;
		short[] temp = new short[4];
		
		for(; round < 11; round++)
		{
			for(int i = 0; i < 4; i++)
				temp[i] = keys[round - 1][i][3];
			
			//shift
			short el = temp[0];
			for(int i = 0; i < 3; i++)
				temp[i] = temp[i + 1];
			temp[3] = el;
			
			//sub
			for(int i = 0; i < 4; i++)
				temp[i] = sBoxValues[temp[i]];
			
			//XOR first byte with RCON value for round
			temp[0] ^= rconValues[round]; 

			//expansion of key
			for(int i = 0; i < 4; i++)
				keys[round][i][0] = (short)(keys[round - 1][i][0] ^ temp[i]);
			for(int i = 1; i < 4; i++)
				for(int j = 0; j < 4; j++)
					keys[round][j][i] = (short)(keys[round][j][i - 1] ^ keys[round - 1][j][i]);
		}
	}

	public short[] encrypt(BigInteger key, String text)
	{

		//intalization
		//////////////////////////////////////////////////////////////////////
		currentBlock = 0;
		start = 0;		

		if(needsPadding(text))
		{
			PAD_LEN = 16 - (text.length() % 16);
			for(int i = 0; i < PAD_LEN; i++)
				text += "\0";
		}
			
		NUM_BLOCKS = text.length() / 16;
		
		//converts the text into a byte array and gets the 
		this.text = toUnsignedShort(text.getBytes());
		
		//this loads the key into the the intial key
		if(TEST)
			loadKey(testKey);
		else
			loadKey(toUnsignedShort(key.toByteArray()));

		//////////////////////////////////////////////////////////////////////
		 		
		short[] ciphertext = new short[this.text.length]; 

		//creates the keys for all ten rounds
		generateKeys();
		
		if(TEST)
		{	
			System.out.println("\nRound Keys:");
			for(int i = 0; i < 11; i++)
			{
				System.out.println("Key [" + i +"]:");
				for(int j = 0; j < 4; j++)
				{
					System.out.printf("[ ");
					for(int k = 0; k < 4; k++)
						if(k != 3)
							System.out.printf("%x, ", keys[i][j][k]);
						else
							System.out.printf("%x ]\n", keys[i][j][k]);
				}
				System.out.println();
			}
		}
		
		int p = 0;
		for(int i = 0; i < NUM_BLOCKS; i++)
		{
			//loads the state for each block
			loadState(this.text);
		
			if(TEST)
			{
				System.out.println("State for block[" + (i + 1) + "]:");
				System.out.println(Arrays.toString(state[0]));
				System.out.println(Arrays.toString(state[1]));
				System.out.println(Arrays.toString(state[2]));
				System.out.println(Arrays.toString(state[3]) + "\n");
			}
			
			//initial round addkey
			addRoundKey(keys[0]);

			//rounds 1 - 9	
			for(int round = 1; round < 10; round++)
			{	
				System.out.println("-----------------------------------------------------------Round = " + round);
				substituteBytes();
				shiftRows();
				mixColumns();
				addRoundKey(keys[round]);
			}
			
			//final round
			substituteBytes();
			shiftRows();
			addRoundKey(keys[10]);	

			//takes the encrypted data and adds it to the ciphertext
			for(int m = 0; m < 4; m++)
			{
				for(int n = 0; n < 4; n++, p++)
				{
					ciphertext[p] = state[n][m];
				}
			}
			
			currentBlock++;
		}

		return ciphertext;
	}

	public short[] encrypt(String key, String text)
	{

		//intalization
		//////////////////////////////////////////////////////////////////////
		currentBlock = 0;
		start = 0;		

		if(needsPadding(text))
		{
			PAD_LEN = 16 - (text.length() % 16);
			for(int i = 0; i < PAD_LEN; i++)
				text += "\0";
		}
			
		NUM_BLOCKS = text.length() / 16;
		
		//converts the text into a byte array and gets the 
		this.text = toUnsignedShort(text.getBytes());
		
		if(key.length() != 16)
		{
			System.out.println("ERROR-Invalid Key Size");
			System.exit(0);
		}

		//this loads the key into the the intial key
		if(TEST)
			loadKey(testKey);
		else
			loadKey(toUnsignedShort(key.getBytes()));

		//////////////////////////////////////////////////////////////////////
		 		
		short[] ciphertext = new short[this.text.length]; 

		//creates the keys for all ten rounds
		generateKeys();
		
		if(TEST)
		{	
			System.out.println("\nRound Keys:");
			for(int i = 0; i < 11; i++)
			{
				System.out.println("Key [" + i +"]:");
				for(int j = 0; j < 4; j++)
				{
					System.out.printf("[ ");
					for(int k = 0; k < 4; k++)
						if(k != 3)
							System.out.printf("%x, ", keys[i][j][k]);
						else
							System.out.printf("%x ]\n", keys[i][j][k]);
				}
				System.out.println();
			}
		}
		
		int p = 0;
		for(int i = 0; i < NUM_BLOCKS; i++)
		{
			//loads the state for each block
			loadState(this.text);
		
			if(TEST)
			{
				System.out.println("State for block[" + (i + 1) + "]:");
				System.out.println(Arrays.toString(state[0]));
				System.out.println(Arrays.toString(state[1]));
				System.out.println(Arrays.toString(state[2]));
				System.out.println(Arrays.toString(state[3]) + "\n");
			}
			
			//initial round addkey
			addRoundKey(keys[0]);

			//rounds 1 - 9	
			for(int round = 1; round < 10; round++)
			{	
				System.out.println("-----------------------------------------------------------Round = " + round);
				substituteBytes();
				shiftRows();
				mixColumns();
				addRoundKey(keys[round]);
			}
			
			//final round
			substituteBytes();
			shiftRows();
			addRoundKey(keys[10]);	

			//takes the encrypted data and adds it to the ciphertext
			for(int m = 0; m < 4; m++)
			{
				for(int n = 0; n < 4; n++, p++)
				{
					ciphertext[p] = state[n][m];
				}
			}
			
			currentBlock++;
		}

		return ciphertext;
	}

	public String decrypt(BigInteger key, short[] cipherbytes)
	{
		//intalization
		//////////////////////////////////////////////////////////////////////
		start = 0;
		currentBlock = 0;		
		NUM_BLOCKS = cipherbytes.length / 16;
		
		//converts the text into a byte array and gets the 
		this.text = cipherbytes;
		
		//this loads the key into the the intial key
		if(TEST)
			loadKey(testKey);
		else
			loadKey(toUnsignedShort(key.toByteArray()));

		//////////////////////////////////////////////////////////////////////
		 		
		byte[] plaintext = new byte[this.text.length]; 

		//creates the keys for all ten rounds
		generateKeys();
		
		if(TEST)
		{	
			System.out.println("\nRound Keys:");
			for(int i = 0; i < 11; i++)
			{
				System.out.println("Key [" + i +"]:");
				for(int j = 0; j < 4; j++)
				{
					System.out.printf("[ ");
					for(int k = 0; k < 4; k++)
						if(k != 3)
							System.out.printf("%x, ", keys[i][j][k]);
						else
							System.out.printf("%x ]\n", keys[i][j][k]);
				}
				System.out.println();
			}
		}
		
		int p = 0;
		for(int i = 0; i < NUM_BLOCKS; i++)
		{
			//loads the state for each block
			loadState(this.text);
		
			if(TEST)
			{
				System.out.println("State for block[" + (i + 1) + "]:");
				System.out.println(Arrays.toString(state[0]));
				System.out.println(Arrays.toString(state[1]));
				System.out.println(Arrays.toString(state[2]));
				System.out.println(Arrays.toString(state[3]) + "\n");
			}
			
			//inverse final round
			addRoundKey(keys[10]);
			inverseShiftRows();
			inverseSubstituteBytes();

			//rounds 9 - 1	
			for(int round = 9; round > 0; round--)
			{	
				System.out.println("-----------------------------------------------------------Round = " + round);
				addRoundKey(keys[round]);
				inverseMixColumns();
				inverseShiftRows();
				inverseSubstituteBytes();
			}
			
			//inverse initial round
			addRoundKey(keys[0]);	

			//takes the encrypted data and adds it to the ciphertext
			for(int m = 0; m < 4; m++)
			{
				for(int n = 0; n < 4; n++, p++)
				{
					plaintext[p] = (byte)state[n][m];
				}
			}
			
			currentBlock++;
		}

		return (new String(plaintext));
	}

	public String decrypt(String key, short[] cipherbytes)
	{
		//intalization
		//////////////////////////////////////////////////////////////////////
		start = 0;
		currentBlock = 0;		
		NUM_BLOCKS = cipherbytes.length / 16;
		
		//converts the text into a byte array and gets the 
		this.text = cipherbytes;
		
		if(key.length() != 16)
		{
			System.out.println("ERROR-Invalid Key Size");
			System.exit(0);
		}

		//this loads the key into the the intial key
		if(TEST)
			loadKey(testKey);
		else
			loadKey(toUnsignedShort(key.getBytes()));

		//////////////////////////////////////////////////////////////////////
		 		
		byte[] plaintext = new byte[this.text.length]; 

		//creates the keys for all ten rounds
		generateKeys();
		
		if(TEST)
		{	
			System.out.println("\nRound Keys:");
			for(int i = 0; i < 11; i++)
			{
				System.out.println("Key [" + i +"]:");
				for(int j = 0; j < 4; j++)
				{
					System.out.printf("[ ");
					for(int k = 0; k < 4; k++)
						if(k != 3)
							System.out.printf("%x, ", keys[i][j][k]);
						else
							System.out.printf("%x ]\n", keys[i][j][k]);
				}
				System.out.println();
			}
		}
		
		int p = 0;
		for(int i = 0; i < NUM_BLOCKS; i++)
		{
			//loads the state for each block
			loadState(this.text);
		
			if(TEST)
			{
				System.out.println("State for block[" + (i + 1) + "]:");
				System.out.println(Arrays.toString(state[0]));
				System.out.println(Arrays.toString(state[1]));
				System.out.println(Arrays.toString(state[2]));
				System.out.println(Arrays.toString(state[3]) + "\n");
			}
			
			//inverse final round
			addRoundKey(keys[10]);
			inverseShiftRows();
			inverseSubstituteBytes();

			//rounds 9 - 1	
			for(int round = 9; round > 0; round--)
			{	
				System.out.println("-----------------------------------------------------------Round = " + round);
				addRoundKey(keys[round]);
				inverseMixColumns();
				inverseShiftRows();
				inverseSubstituteBytes();
			}
			
			//inverse initial round
			addRoundKey(keys[0]);	

			//takes the encrypted data and adds it to the ciphertext
			for(int m = 0; m < 4; m++)
			{
				for(int n = 0; n < 4; n++, p++)
				{
					plaintext[p] = (byte)state[n][m];
				}
			}
			
			currentBlock++;
		}

		return (new String(plaintext));
	}

	private void addRoundKey(short[][] key)
	{
		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before addRoundKey[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}

		for(int i = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++)
			{
				state[i][j] ^= key[i][j];
			}
		}

		if(TEST)
		{
			System.out.println("State for block after addRoundKey[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	}
	
	private void substituteBytes()
	{

		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before substituteBytes[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}

		for(int i = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++)
			{
				state[i][j] = sBoxValues[state[i][j]];
			}
		}

		if(TEST)
		{
			System.out.println("State for block after substituteBytes[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	} 

	private void inverseSubstituteBytes()
	{
		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before inverseSubstituteBytes[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}

		for(int i = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++)
			{
				state[i][j] = inverseSbox[state[i][j]];
			}
		}

		if(TEST)
		{
			System.out.println("State for block after inverseSubstituteBytes[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	}

	private void shiftRows()
	{
		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before shiftRows[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}
		//the zeroth row is not shifted
		
		//the 1st row is shfited one to the left
		short temp = state[1][0];
		for(int start = 1; start < 4; start++)
			state[1][start - 1] = state[1][start];
		state[1][3] = temp;
		
		//the 2nd row is shifted two to the left
		temp = state[2][0];
		short temp2 = state[2][1];
		for(int start = 2; start < 4; start++)
			state[2][start - 2] = state[2][start];
		state[2][2] = temp;
		state[2][3] = temp2;
		
		//the 3rd row is shifted three to the left
		temp = state[3][0];
		temp2 = state[3][1];
		short temp3 = state[3][2];
		for(int start = 3; start < 4; start++)
			state[3][start - 3] = state[3][start];
		state[3][1] = temp;
		state[3][2] = temp2;
		state[3][3] = temp3;
		
		if(TEST)
		{
			System.out.println("State for block after shiftRows[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	}
	
	private void inverseShiftRows()
	{

		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before inverseShiftRows[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}
		short temp, temp2, temp3, temp4;
		//nothing happens to the first row
		
		//1st row gets shifted one to the right
		temp = state[1][0];
		temp2 = state[1][1];
		temp3 = state[1][2];
		temp4 = state[1][3];

		state[1][0] = temp4;
		state[1][1] = temp;
		state[1][2] = temp2;
		state[1][3] = temp3;

		//2nd row gets shifted two to the right
		temp = state[2][0];
		temp2 = state[2][1];
		temp3 = state[2][2];
		temp4 = state[2][3];

		state[2][0] = temp3;
		state[2][1] = temp4;
		state[2][2] = temp;
		state[2][3] = temp2;

		//3rd row get shifted three to the right
		temp = state[3][0];
		temp2 = state[3][1];
		temp3 = state[3][2];
		temp4 = state[3][3];	
	
		state[3][0] = temp2;
		state[3][1] = temp3;
		state[3][2] = temp4;
		state[3][3] = temp;
			
		if(TEST)
		{
			System.out.println("State for block after inverseShiftRows[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	}

	private void mixColumns()
	{
		short[][] temp = new short[4][4];

		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before mixColumns[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}
		
		for(int j = 0; j < 4; j++)
		{
			temp[0][j] = (short)(galoisMultiply(state[0][j], 2) ^ galoisMultiply(state[1][j], 3) ^ state[2][j] ^ state[3][j]);
			temp[1][j] = (short)(state[0][j] ^ galoisMultiply(state[1][j], 2) ^ galoisMultiply(state[2][j], 3) ^ state[3][j]);
			temp[2][j] = (short)(state[0][j] ^ state[1][j] ^ galoisMultiply(state[2][j], 2) ^ galoisMultiply(state[3][j], 3));
			temp[3][j] = (short)(galoisMultiply(state[0][j], 3) ^ state[1][j] ^ state[2][j] ^ galoisMultiply(state[3][j], 2));
		} 
		
		for(int i = 0; i < 4; i++)
		{
			state[i] = Arrays.copyOf(temp[i], 4);
		}
		
		if(TEST)
		{
			System.out.println("State for block after mixColumns[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	}

	private void inverseMixColumns()
	{
		if(TEST)
		{
			System.out.println("------------------------------------------------------------");
			System.out.println("State for block before inverseMixColumns[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]) + "\n");
		}

		final int SIZE = 4;
		short[] a = new short[SIZE];  

		for( int j = 0; j < SIZE; j++ )
		{
	
			for( int i = 0; i < SIZE; i++)
					a[i] = state[i][j];

			state[0][j] = (short) (galoisMultiply(a[0],14) ^ galoisMultiply(a[3],9) ^ galoisMultiply(a[2],13) ^ galoisMultiply(a[1],11));
			state[1][j] = (short) (galoisMultiply(a[1],14) ^ galoisMultiply(a[0],9) ^ galoisMultiply(a[3],13) ^ galoisMultiply(a[2],11));
			state[2][j] = (short) (galoisMultiply(a[2],14) ^ galoisMultiply(a[1],9) ^ galoisMultiply(a[0],13) ^ galoisMultiply(a[3],11));
			state[3][j] = (short) (galoisMultiply(a[3],14) ^ galoisMultiply(a[2],9) ^ galoisMultiply(a[1],13) ^ galoisMultiply(a[0],11));
		 }
		
		if(TEST)
		{
			
			System.out.println("State for block after MixColumns[" + (currentBlock + 1) + "]:");
			System.out.println(Arrays.toString(state[0]));
			System.out.println(Arrays.toString(state[1]));
			System.out.println(Arrays.toString(state[2]));
			System.out.println(Arrays.toString(state[3]));
			System.out.println("------------------------------------------------------------");
		}
	}

	private short galoisMultiply(int a, int b)
	{
		int p = 0;	
		int highBit;
	
		for( int i = 0; i < 8; i++)
		{
			if((b & 1) == 1) 
				p ^= a;
			highBit = a & 0x80;
			a <<= 1;
			if(highBit == 0x80) 
				a ^= 0x1b;		
			b >>= 1;
		}
	
		p &= 0xff;
	
		return (short)p;
	}

	private short[] toUnsignedShort(byte[] bytes)
	{
		short[] array = new short[bytes.length];
		short adjust = 256;		
		
		for(int i = 0; i < bytes.length; i++)
			if(bytes[i] >= 0)
				array[i] = bytes[i];
			else
				array[i] = (short)(bytes[i] + adjust);
		
		return array;
	}

	private boolean needsPadding(String t){return (t.length() % 16 == 0)?false:true;}
}
